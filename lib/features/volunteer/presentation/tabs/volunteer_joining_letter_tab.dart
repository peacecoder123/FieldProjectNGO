...
class _JoiningLetterCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final statusBgColor = switch (letter.status) {
      RequestStatus.approved     => AppColors.emerald50,
      RequestStatus.waitingAdmin => AppColors.brand.withValues(alpha: 0.05),
      RequestStatus.rejected     => AppColors.red50,
      RequestStatus.pending      => AppColors.orange50,
    };
    ...
    final statusIcon = switch (letter.status) {
      RequestStatus.approved     => Icons.check_circle_rounded,
      RequestStatus.waitingAdmin => Icons.admin_panel_settings_rounded,
      RequestStatus.rejected     => Icons.cancel_rounded,
      RequestStatus.pending      => Icons.schedule_rounded,
    };
          ...
          Expanded(
            child: Column(
              ...
              children: [
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: Row(
                          children: [
                            const Icon(Icons.info_outline_rounded, size: 16, color: AppColors.emerald700),
                            const SizedBox(width: 8),
                            Expanded(
                              child: Text(
                                'This letter is now available for download in the Certificate tab.',
                                style: TextStyle(
                                  fontSize: 12,
                ...
                if (letter.status == RequestStatus.pending || letter.status == RequestStatus.waitingAdmin)
                  Padding(
                    padding: const EdgeInsets.only(top: 8),
                    child: Row(
                      children: [
                        Icon(
                          letter.status == RequestStatus.pending ? Icons.schedule_rounded : Icons.admin_panel_settings_rounded,
                          size: 12,
                          color: letter.status == RequestStatus.pending ? AppColors.amber600 : AppColors.brand,
                        ),
                        const SizedBox(width: 4),
                        Text(
                          letter.status == RequestStatus.pending ? 'Awaiting mentor review' : 'Awaiting admin review',
                          style: TextStyle(
                            fontSize: 12,
                            color: letter.status == RequestStatus.pending ? AppColors.amber600 : AppColors.brand,
                            fontWeight: FontWeight.w500,
