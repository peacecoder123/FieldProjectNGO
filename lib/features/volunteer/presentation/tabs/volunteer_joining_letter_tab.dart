                ...
                if (letter.status == RequestStatus.approved && letter.generatedBy != null)
                  Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 8),
                      ElevatedButton.icon(
                        onPressed: () => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Letter downloaded!'), backgroundColor: AppColors.emerald500)),
                        icon: const Icon(Icons.download_rounded, size: 16),
                        label: const Text('Download Letter'),
                        style: ElevatedButton.styleFrom(backgroundColor: AppColors.emerald50, foregroundColor: AppColors.emerald600, elevation: 0, padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8)),
                      ),
                    ],
                  ),
                if (letter.status == RequestStatus.pending)
